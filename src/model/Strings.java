...
public class Strings {
	 ...
	 * characters long and not beginning with space. Too long
	 * Strings cannot be saved in the MongoDB, which supports only 16 MB
	 * documents, and to short Strings like one character or empty, may disturb
	 * some methods. Also String containing only spaces will return an error
	 * from IBM Api, so the first character must not be a space.
	 * 
	 * @param name
	 *            is the String inputed by the user
	 * @return the conclusion, true or false (true - to be accepted for further
	 *         processing or false to be rejected and to trigger an error
	 *         prompt
	 */
	public boolean verifyString(String name) {
//		boolean conclusion;
//		if ((name.length() < 1 || name.length() >= 80) || name.charAt(0) == ' ') {
//			conclusion = false;
//		} else {
//			conclusion = true;
//		}
